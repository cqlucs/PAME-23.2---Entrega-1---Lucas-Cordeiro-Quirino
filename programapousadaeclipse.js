...
class usuario {
    constructor (id, nome, email, hist, senha) {
        this.id = id;
        this.nome = nome;
        this.email = email;
        this.hist = hist;
...
class sistema {
    ...
    fornecerindexuserlogado() {
        //Fornece o index do usuário logado
        if (this.listausers.length == 1){
            return 0;
        }
        else {
    ...
    while (baseloop == 1) {
        ...
        if (opcao == "1") {
            ...
            if (busca == 0) {
                ...
                //Usuário existe, solicitar senha
                let senhaacesso = req.question("Digite sua senha para acessar a conta:\n");
                if (busca.senha == senhaacesso) {
                    //Senha correta, login aprovado
                    sist.logar(busca);
                    ...
                    //Senha incorreta
                    console.log("Opa! Senha incorreta, por favor, verifique se inseriu a senha corretamente!");
                }
            }
        }
        else if (opcao == "2") {
            ...
            if (busca == 0) {
                ...
                let senhauser = req.question("Digite uma senha para acessar sua conta:\n");
                let novocadastro =  new usuario(sist.novoiduser(), usuarionov, emailinf, [], senhauser);
        ...
        while (baseloop == 2) {
            ...
            if (opcao == "1"){
                ...
                let exbsenha = req.question("A senha é um dado importante, deseja exibir? s/n");
                if (exbsenha == "s") {
                    console.log("Senha do usuário:", sist.usuariolog.senha);
                }
                else {
                    console.log("Senha escondida.")
                }
                ...
                console.log("3 - Senha do usuário");
                ...
                if (opcao1 == "1") {
                    ...
                    sist.listausers[sist.fornecerindexuserlogado()].alterarnome(novonome);
                    ...
                    sist.listausers[sist.fornecerindexuserlogado()].alteraremail(novoemail);
                    sist.usuariolog.alteraremail(novoemail);
                    console.log("Email do usuário alterado com sucesso!")
                    req.question("Pressione ENTER para continuar...");
                }
                else if (opcao1 == "3") {
                    //Caso o usuário deseje alterar o email
                    let novasenha = req.question("Digite a nova senha do usuário:\n");
                    sist.listausers[sist.fornecerindexuserlogado()].alterarsenha(novasenha);
                    sist.usuariolog.alterarsenha(novasenha);
                ...
                if (sist.listaprops.length == 0) {
                    //Nenhuma propriedade para exibir, mensagem informativa
                    console.log("Nenhuma propriedade está registrada.");
                    req.question("Pressione ENTER para continuar...");
                }
                else {
                ...
                if (sist.listareservas.length == 0) {
                    //Nenhuma reserva para exibir, mensagem informativa
                    console.log("Nenhuma reserva está registrada.");
                    req.question("Pressione ENTER para continuar...");
                }
                else {
                ...
                if (sist.listaanuncios.length == 0) {
                    //Nenhum anúncio para exibir, mensagem informativa
                    console.log("Nenhum anúncio está registrado.");
                    req.question("Pressione ENTER para continuar...");
                }
                else {
                ...
                if (sist.listaprops.length == 0) {
                    ...
                    if (propbusca == 0) {
                        ...
                        //Agora informações adicionais, valor da reserva é calculado manualmente, status do pagamento é solicitado depois
                        //let valorres = req.question("Insira o valor total da reserva:\n"); Método manual, presente na versão da entrega parcial
                        //Fazer o cálculo do valor total da reserva numnoites*preconoite, primeiro verificar o número de noites
                        let numnoites = 0;
                        if (anocout > anocin) {
                            //Caso a reserva dure anos
                            numnoites = ((anocout-anocin)*365);
                            //Verificar ainda se é menos de um ano ou mais
                            if (mescout > mescin) {
                                numnoites += ((mescout-mescin)*30);
                                if (diacout > diacin) {
                                    numnoites += (diacout-diacin);
                                }
                                else if (diacout < diacin) {
                                    numnoites -= (diacin-diacout);
                                }
                            }
                            else if (mescout == mescin) {
                                if (diacout > diacin) {
                                    numnoites += (diacout-diacin);
                                }
                                else if (diacout < diacin) {
                            ...
                            //Caso a reserva dure meses, 30 dias como aprox
                            numnoites = ((mescout-mescin)*30);
                            //Verificar ainda se é menos de um mês ou mais
                            if (diacout > diacin) {
                                numnoites += (diacout-diacin);
                            }
                            else if (diacout < diacin) {
                            ...
                            //Caso a reserva dure dias
                            numnoites = diacout-diacin;
                        }
                        else {
                            //Caso não passe uma noite, apenas fique um dia, colocar o valor padrão de 1 noite
                            numnoites = 1;
                        }
                        let valorres = propbusca.noite*numnoites;
                        console.log("O valor total da reserva é:", valorres);
                ...
                if (sist.listaprops.length == 0) {
                    //Nenhuma propriedade para exibir, mensagem informativa
                    console.log("Nenhuma propriedade está registrada para se fazer um anúncio.");
                    req.question("Pressione ENTER para continuar...");
                }
                else {
                ...
                if (sist.listaprops.length == 0) {
                    //Nenhuma propriedade para exibir, mensagem informativa
                    console.log("Nenhuma propriedade está registrada para ser avaliada.");
                    req.question("Pressione ENTER para continuar...");
                }
                else {
                ...
                if (sist.listaavaliacoes.length == 0) {
                    //Nenhuma avaliação para exibir, mensagem informativa
                    console.log("Nenhuma avaliação está registrada.");
                    req.question("Pressione ENTER para continuar...");
                }
                else {
